...
namespace XlsMerger
{
	class ChukuSheetReader
	{
		...
		private string[] headers = { "出货单日期", "出货单号", "客户名称", "发票号", "商品名称", "规格型号", "单位", "数量", "单价", "金额", "税额"};
		...
		private void readSheetFile(FileStream file, string filePath)
		{
			...
			while (rows.MoveNext())
			{
				...
				if (cell != null)
				{
					entity.ck_djjs = cell.ToString().Trim();
					//金额
					entity.ck_djje = Math.Round((decimal.Parse(entity.ck_djjs) / 1.17m), 2).ToString();
					//税额
					entity.ck_djse = Math.Round(decimal.Parse(entity.ck_djjs) - decimal.Parse(entity.ck_djje), 2).ToString();
				}
				//备注
				/*
				cell = row.GetCell(8);
